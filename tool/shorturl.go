...
func generator6(charset, url, hexMd5 string, sectionNum int) string {
...
// 生成8字符短key
func generator8(charset, url, hexMd5 string, sectionNum int) string {
	for i := 0; i < sectionNum; i++ {
...
// Generator 生成6-8字符的短链接，参数t表示字符集类型，回调函数(cb)用于检测短链接是否重复
// 起初生成6位的短链接，当四组6位短链接都重复时，再生成8位的短链接
func Generator(t int, domain string, url string) string {
	if domain == "" || url == "" {
		return ""
	}
	...
	keyword := generator6(charset, url, hexMd5, sections)
	if keyword == "" {
		keyword = generator8(charset, url, hexMd5, sections)
		if keyword == "" {
			return ""
		}
	}
